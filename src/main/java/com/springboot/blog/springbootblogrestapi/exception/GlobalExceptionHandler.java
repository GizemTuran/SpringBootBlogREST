...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
                                                                  ...
                                                                  WebRequest request) {
        Map<String, String> errors = new HashMap<>(); //Stores the error messages that matches with fieldname.
        ex.getBindingResult().getAllErrors().forEach((error)->{ //gets the errors during the verification
            String fieldName = ((FieldError)error).getField();
            String message = error.getDefaultMessage(); //gets the error message
   ...
   /* @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<Object> handleResourceNotFoundException(MethodArgumentNotValidException exception,
                                                                        WebRequest webRequest){
        Map<String, String> errors = new HashMap<>(); //Stores the error messages that matches with fieldname.
        exception.getBindingResult().getAllErrors().forEach((error)->{ //gets the errors during the verification
            String fieldName = ((FieldError)error).getField();
            String message = error.getDefaultMessage(); //gets the error message
            errors.put(fieldName,message);
        });
